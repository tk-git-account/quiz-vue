...
new Vue({
  ...
  data: {
    ...
    correctNum: 0,
    shuffledArr: [],
    quizNum: Number(location.hash.substring(1))
  ...
  methods: {
    ...
    nextQuiz: function () {
      ...
      //if (this.questionIndex === this.questions.length) {  木村修正
        if (this.questionIndex === this.quizNum) {
    ...
    jampSelectMenu: function(){
      window.location.href = './selectMenu.html'; // 通常の遷移
    },
    jampIndex: function(){
    ...
    initializeApp: function () {
      ...
      // 問題をランダムに並べた配列を作成する
      //var questionsNum = this.questions.length;　//木村削除
      var questionsNum = this.quizNum;　//木村追加
      //console.log('quizNum : %s',this.quizNum);　//木村追加
